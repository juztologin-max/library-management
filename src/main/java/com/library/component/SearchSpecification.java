	...
	public @Nullable Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder)
		...
		for (Map.Entry<String, JsonNode> entry : this.jsonNode.properties()) {
			...
			case "Between" -> {
				...
				predicateList.add(criteriaBuilder.between(column.as((Class<? extends Comparable>) type),
						(Comparable) conversionService.convert(firstTerm, type),
						(Comparable) conversionService.convert(secondTerm, type)));
			}
			default -> {
